...
# Reuse existing nodebuilder-flatnotes container, to speed up builds
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-flatnotes; then
    echo "Pulling NodeJS runtime..."
    buildah from --name nodebuilder-flatnotes -v "${PWD}:/usr/src:Z" docker.io/library/node:lts
fi
...
buildah config --entrypoint=/ \
    ...
    --label="org.nethserver.images=docker.io/dullage/flatnotes:latest" \
    "${container}"
# Commit the image
